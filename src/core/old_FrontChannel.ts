...
export class FrontChannel extends Channel {
    ...
    //subscrition handler lookup
    private subscriptionHandlerIds: Object<string, number>;
    ...
    public send(message: any, backChannelId=this.channelId) : void {
        this.channelMessageHandlers.get(backChannelId).send(this.frontUid, message);
    }
    ...
    private initializePreConnectSubs() : void {
        ...
        */
        ...
        this.centrum.createSubscription(Protocol.CONNECT_SUCCESS(this.frontUid), (data: { frontUid, channelId }) => {
    ...
    private initializePreConnectedPubs() : void {
        ...
        this.connectHandler = this.centrum.getOrCreatePublish(Protocol.CONNECT());
        this.broadcastAllHandler = this.centrum.getOrCreatePublish(Protocol.BROADCAST_ALL_BACK());
    }
    ...
    private onConnected(backChannelId) {
        ...
        if(backChannelId === this.channelId) {
            ...
            this.subs.create.patchState(this.channelId, )
